...
struct tm *
gmtime_r (const time_t * const timep,
	  struct tm * tmp)
{
...
char *
ctime_r(const time_t * const	timep,
	char *			buf)
{
	struct tm	mytm;
...
static int
long_increment_overflow (long *	number,
			 int	delta)
{
	long	number0;
...
static int
normalize_overflow (int * const	tensptr,
		    int * const	unitsptr,
		    const int	base)
{
	int	tensdelta;
...
static int
long_normalize_overflow (long * const	tensptr,
			 int * const	unitsptr,
			 const int	base)
{
